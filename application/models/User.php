<?php
    ...
    class User extends CI_Model{ 
        ...
        public function getProfileData($data){
            $this->db->select('id, name, email, city, phone, face_photo, side_face_photo, full_body_photo, tool_photo, otp_verified, work_location, lat, lng, primary_profession, sub_profession, mode_of_transport, device_id, created_at');
            $query = $this->db->get_where('worker', array('id' => $data))->row();
            return $query;
        }

        public function getCustomerData($data){
            $this->db->select('id, name, email, phone, photo, address_1, address_2, city, state, pincode, otp_verified, referral, device_id');
